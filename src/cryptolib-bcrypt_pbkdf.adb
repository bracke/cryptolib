...
package body CryptoLib.BCrypt_PBKDF is
   ...
   subtype Byte_Value is Ada.Streams.Stream_Element;
   ...
   is
   begin
      Clear_P_Array (Item.P_Data);
      for Index_Value in Item.S_Data'Range loop
         Item.S_Data (Index_Value) := 0;
      end loop;
   end Clear_State;

   Initial_P : constant P_Array :=
     [16#243F6A88#,
      16#85A308D3#,
      16#13198A2E#,
      16#03707344#,
   ...
   function To_Word_32 (Value : Ada.Streams.Stream_Element) return Word_32
     with SPARK_Mode => On
   is
   begin
      return Word_32 (Value);
   end To_Word_32;

   function Byte_From
     (Data : Ada.Streams.Stream_Element_Array; Offset_Value : Natural)
      return Byte_Value
     with SPARK_Mode => On,
          Pre => Data'First <= Data'Last
            and then (if Data'First > 0
                      then Ada.Streams.Stream_Element_Offset (Offset_Value)
                        <= Ada.Streams.Stream_Element_Offset'Last - Data'First
                      else True)
            and then Data'First + Ada.Streams.Stream_Element_Offset (Offset_Value) <= Data'Last
   is
   begin
      return
   ...
   begin
      for Index_Value in 1 .. 4 loop
         ...
         Result :=
           ...
           or To_Word_32 (Byte_From (Data, Offset_Value));
         Offset_Value := Offset_Value + 1;
         if Offset_Value >= Data'Length then
            Offset_Value := 0;
         end if;
      end loop;
      return Result;
   ...
   function F_Value
     (State_Item : Blowfish_State; Word_Value : Word_32) return Word_32
   is
      A_Index : constant Natural :=
        Natural (Interfaces.Shift_Right (Word_Value, 24) and 16#FF#);
      B_Index : constant Natural :=
        Natural (Interfaces.Shift_Right (Word_Value, 16) and 16#FF#);
      C_Index : constant Natural :=
        Natural (Interfaces.Shift_Right (Word_Value, 8) and 16#FF#);
      D_Index : constant Natural := Natural (Word_Value and 16#FF#);
   begin
      return
        ((State_Item.S_Data (A_Index) + State_Item.S_Data (256 + B_Index))
         xor State_Item.S_Data (512 + C_Index))
        + State_Item.S_Data (768 + D_Index);
   end F_Value;

   procedure Encipher
     (State_Item : Blowfish_State;
      Left_Word  : in out Word_32;
      Right_Word : in out Word_32)
   is
      Left_Value  : Word_32 := Left_Word xor State_Item.P_Data (0);
      Right_Value : Word_32 := Right_Word;
      Swap_Value  : Word_32;
   begin
      for Round_Index in 1 .. 16 loop
         if Round_Index mod 2 = 1 then
            Right_Value :=
              Right_Value
              xor F_Value (State_Item, Left_Value)
              xor State_Item.P_Data (Round_Index);
         else
            Left_Value :=
              Left_Value
              xor F_Value (State_Item, Right_Value)
              xor State_Item.P_Data (Round_Index);
         end if;
      end loop;

      Swap_Value := Left_Value;
      Left_Word := Right_Value xor State_Item.P_Data (17);
      Right_Word := Swap_Value;
   end Encipher;
   ...
   is
   begin
      State_Item.P_Data := Initial_P;
      State_Item.S_Data := Initial_S;
   end Init_State;
   ...
   is
      ...
      Salt_Offset : Natural := 0;
      Left_Value  : Word_32 := 0;
      Right_Value : Word_32 := 0;
   begin
      for Index_Value in State_Item.P_Data'Range loop
         State_Item.P_Data (Index_Value) :=
           State_Item.P_Data (Index_Value)
           xor Stream_To_Word (Key_Data, Key_Offset);
      end loop;

      for Index_Value in 0 .. (P_Count / 2) - 1 loop
         Left_Value := Left_Value xor Stream_To_Word (Salt_Data, Salt_Offset);
         Right_Value :=
           Right_Value xor Stream_To_Word (Salt_Data, Salt_Offset);
         Encipher (State_Item, Left_Value, Right_Value);
         State_Item.P_Data (Index_Value * 2) := Left_Value;
         State_Item.P_Data (Index_Value * 2 + 1) := Right_Value;
      end loop;

      for Index_Value in 0 .. (S_Count / 2) - 1 loop
         Left_Value := Left_Value xor Stream_To_Word (Salt_Data, Salt_Offset);
         Right_Value :=
           Right_Value xor Stream_To_Word (Salt_Data, Salt_Offset);
         Encipher (State_Item, Left_Value, Right_Value);
         State_Item.S_Data (Index_Value * 2) := Left_Value;
         State_Item.S_Data (Index_Value * 2 + 1) := Right_Value;
      end loop;
   end Expand_State;
   ...
   is
      ...
      Right_Value : Word_32;
   begin
      for Pair_Index in 0 .. Pair_Count - 1 loop
         Left_Value := Data_Words (Pair_Index * 2);
         Right_Value := Data_Words (Pair_Index * 2 + 1);
         Encipher (State_Item, Left_Value, Right_Value);
         Data_Words (Pair_Index * 2) := Left_Value;
         Data_Words (Pair_Index * 2 + 1) := Right_Value;
      end loop;
   end Encrypt_Words;
