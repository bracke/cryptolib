...
package body CryptoLib.X509.Certificates is
   ...
   --  Turn UTCTime or GeneralizedTime into its parts.
   --
   --  RFC 5280 pins both to UTC with seconds present and a trailing Z, which
   --  is narrower than ASN.1 allows. Accepting the wider forms would mean
   --  guessing a zone, and a validity window guessed wrong is a certificate
