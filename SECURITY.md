...
| Finite-field DH | groups 1 / 14 / 16 / 18 | live vs OpenSSH; group16/18 pin the exact RFC 3526 primes; group14 is pinned by a fixed exponent against a written-down answer, since a round trip between two honest sides is self-consistent and would pass with the wrong prime; group14 and group16 both refuse a peer public value outside (1, p-1) -- zero and one yield a shared secret of zero or one whatever the private exponent is, which a round-trip test cannot notice |
