...
| Password hashing | Argon2d / Argon2i / Argon2id (RFC 9106) | against libargon2, the reference implementation, driven through its `argon2_ctx` entry point so the secret and associated-data inputs are covered too. The three RFC 9106 section 5 vectors are reproduced exactly, and libargon2 was confirmed to produce those published tags before anything was compared against it. Beyond them: single-lane runs at 64 MiB for all three variants, a four-lane run that exercises the cross-lane reference path a single lane cannot reach, and a 128-octet tag that drives the H' construction past its 64-octet chain boundary. Two errors were found this way and are worth naming, because small parameters hide both -- the reference area for a same-lane block excludes the immediately preceding block, and the data-independent address slot is the segment index itself rather than a running counter, which agrees with a counter only while a segment is shorter than 128 blocks. Out-of-range parameters are refused with the tag left zero: a salt under 8 octets, a tag under 4, memory below 8 blocks per lane  `Verify` compares through `Constant_Time.Equal`, which `Derive` alone left a caller to get right: the obvious `Tag = Stored` stops at the first differing octet, and that is a timing oracle on the value the hash exists to protect. Bcrypt had this and Argon2 did not, which was the wrong way round given Argon2 is the one recommended here. Checked to accept the password that produced the tag and to refuse a wrong password, a wrong variant, a wrong cost, a wrong pepper, and a tag differing only in its first or only in its last octet -- the last of those being what an early-exit comparison gets wrong. A declined derivation returns False without comparing, so a refused parameter cannot read as a match against the all-zero tag `Derive` leaves behind; removing that guard fails the suite.  `Hash` and `Verify_Encoded` carry the PHC string -- `$argon2id$v=19$m=65536,t=3,p=4$salt$tag` -- so a stored hash describes its own costs and keeps verifying after they are raised; the encoding is checked byte for byte against argon2-cffi, which wraps the reference implementation, for all three variants and for a 16-octet tag at p=4. Parsing is deliberately strict, because a stored string is compared as text and must have one spelling: an unknown variant, v=16 (whose indexing this does not implement), a missing or repeated cost, a cost written with a leading zero, base64 padding, and a missing tag are each refused rather than guessed at. |
...
| ECDSA | P-256 / P-384 / P-521 sign and verify; the public point is checked to be on the curve and within the field before use | **RFC 6979 A.2.5** (P-256 and P-384, byte-exact -- for P-256 both the `sample` and `test` messages, since the DRBG state width is the paired digest's and a vector that only asked whether *a* signature came out would pass on a curve that cannot sign at all) + P-521 (pyca cross-verified); a generated P-256 keypair is checked to be a pair -- the point the scalar implies, not merely a point -- and then signed and verified with; verification against OpenSSL signatures on all three curves, including curve/digest pairings that differ (P-521 with SHA-256, P-384 with SHA-512), and separately through the `Nistp*_Raw` entry points, which choose the digest themselves rather than taking it from the caller -- the pairing SSH means by `ecdsa-sha2-nistp256`, checked against signatures made elsewhere over the matching hash, so a curve wired to the wrong one fails here rather than only against the rest of the world  `Encode_DER_Signature` turns the raw (r, s) the signers return into the `SEQUENCE { r INTEGER, s INTEGER }` that X.509 and TLS carry -- the encoding certificate issuance built inline, now shared and reachable by a caller signing with a client certificate. Each INTEGER is minimal: leading zeros dropped, a 16#00# restored when the top bit would otherwise read as negative. Checked against five signatures OpenSSL produced, taken apart into r and s and required to encode back byte for byte, covering the padded case, the unpadded case, P-521's long-form SEQUENCE length and a leading zero octet that must be dropped. Removing the padding, the stripping, or the long form each fails the suite. Certificate issuance now goes through it, so its own OpenSSL chain cross-check covers it too. |
