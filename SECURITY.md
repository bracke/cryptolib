...
| ML-DSA (FIPS 204) | key generation, all three parameter sets | against NIST's ACVP vectors from usnistgov/ACVP-Server, two seeds per parameter set, comparing a SHA-256 of the encoded public and private keys because the keys themselves run to kilobytes. The reference used while building it was validated first: dilithium-py reproduces all 75 ACVP keyGen vectors, so it was a checked reference rather than an assumed one. The NTT constants were derived from zeta = 1753 and checked to be a primitive 512th root of unity rather than transcribed. **Signing and verification are not implemented**, so this cannot yet produce or check a signature |
